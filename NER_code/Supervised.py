...
val_set, train_set = torch.utils.data.random_split(train_set, [size_val, size_train], generator=torch.Generator().manual_seed(87))

test_dataloader = DataLoader(test_set, batch_size=128, shuffle=False, collate_fn=collate_object)
validation_dataloader = DataLoader(val_set, batch_size=128, shuffle=False, collate_fn=collate_object)
...
for epoch in range(supervised_epochs):
    ...
    if val_loss > smallest_loss:
        ...
        if patience == 0 or val_loss > 1.3*smallest_loss:
            break
    else:
